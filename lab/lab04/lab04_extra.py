...
def put_piece(board, max_rows, column, player):
    ...
    idx = max_rows - 1
    while idx >= 0:
        if get_piece(board, idx, column) == '-':
            break
        idx -= 1
    if  idx < 0:
        return -1, board
    else:
        return idx, replace_elem(board, idx, replace_elem(board[idx], column, player)) 
