...
(define (larger-values x lst)
  (filter (lambda (element) (> element x)) lst)
)
...
(define (derive-sum expr var)
  (define derive-addend (derive (addend expr) var))
  (define derive-augend (derive (augend expr) var))
  (make-sum derive-addend derive-augend)
)

(define (derive-product expr var)
  (define derive-multiplier
          (derive (multiplier expr) var)
  )
  (define derive-multiplicand
          (derive (multiplicand expr) var)
  )
  (make-sum (make-product derive-multiplier
                          (multiplicand expr)
            )
